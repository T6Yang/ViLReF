...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        self.self = BertSelfAttention(config) if not config.use_flash_attention else FlashMHA(config.hidden_size,
                                                                                              config.num_attention_heads)
        self.output = BertSelfOutput(config) if not config.use_flash_attention else BertSelfOutputForFlashAttention(
            config)
