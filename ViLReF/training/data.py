...
from ViLReF.clip import _tokenizer
from ViLReF.clip import tokenize
...
class LMDBDataset(Dataset):
    def __init__(self, lmdb_path, split="val", unpair=False, max_txt_length=64, use_augment=False, resolution=224):
    ...
    def _build_transform(self, resolution):
        if self.split == "train" and self.use_augment:
            transform = create_transform(
                             ...
                             scale=(0.9, 1.0),
                             is_training=True,
                             color_jitter=0.1,  # no hue changed
                             hflip=0.5,
                             vflip=0.,
                            #  auto_augment='original', # set to None
    ...
    def __getitem__(self, index):
        ...
        image_id, text_id, raw_text, label = pair
        ...
        text = tokenize([_preprocess_text(raw_text[3:])], context_length=self.max_txt_length)[0]
        self.tmp += 1

        label_image = torch.tensor(label).float()
        label_text = torch.tensor(label).float()
        eos_index = text.numpy().tolist().index(_tokenizer.vocab['[SEP]'])
        return image, text, label_image, label_text, eos_index, index
...
def get_dataset(args, is_train, unpair=False, max_txt_length=64, epoch_id=0):
    if is_train:
...
def get_data(args, unpair, epoch_id=0, max_txt_length=64):
    data = {}
