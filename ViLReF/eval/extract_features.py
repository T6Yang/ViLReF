...
sys.path.append("/home/yangsz/myProject/6disease/AD_text_to_image/text2image-main/cn_clip/DAMSM_with_cn_clip_v2/cn_clip")
        ...
        with open(args.text_feat_output_path, "w") as fout:
            ...
            with torch.no_grad():
                for batch in tqdm(dataloader):
                    text_ids, texts, texts_length = batch
                    texts = texts.cuda(args.gpu, non_blocking=True)
                    word_features, text_features = model(None, texts)
                    text_features /= text_features.norm(dim=-1, keepdim=True)
                    word_features /= word_features.norm(dim=-1, keepdim=True)
                    word_features = word_features.transpose(1, 2)

                    for text_id, text_feature, word_feature, texts_length in zip(text_ids.tolist(), text_features.tolist(), word_features.tolist(), texts_length.tolist()):
                        fout.write("{}\n".format(json.dumps({"text_id": text_id, "text_length": texts_length, "text_feature": text_feature, "word_feature": word_feature})))
                        write_cnt += 1
        ...
        with open(args.image_feat_output_path, "w") as fout:
            ...
            with torch.no_grad():
                for batch in tqdm(dataloader):
                    ...
                    patch_features, image_features = model(images, None)
                    image_features /= image_features.norm(dim=-1, keepdim=True)
                    patch_features /= patch_features.norm(dim=-1, keepdim=True)
                    for image_id, image_feature, patch_feature in zip(image_ids.tolist(), image_features.tolist(), patch_features.tolist()):
                        fout.write("{}\n".format(json.dumps({"image_id": image_id, "image_feature": image_feature, "patch_feature": patch_feature})))
                        write_cnt += 1
